...
namespace KlamathIrrigationDistrict.Controllers
{
    public class DitchRidersController : Controller
    {
         ...
         * Page: EditRequestOn*/
        [Authorize]
        public ActionResult EditRequestOn(int id, int RequestID,  string lateral)
        {
            if (!User.IsInRole("Ride " + id.ToString()) && !User.IsInRole("Relief Ride " + id.ToString()))
            {
                return View("Unauthorized");
        ...
        {
            var ditchriderrequests = _ditchRiderRepo.Customers(id).Where(s => s.Structure == Structure).FirstOrDefault();
            if (!User.IsInRole("Ride " + id.ToString()) && !User.IsInRole("Relief Ride " + id.ToString()))
            {
                return View("Unauthorized");
            }            
